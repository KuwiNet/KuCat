...
30 4 * * * TMP=/tmp/kucat-\$\$; git clone --depth 1 $REPO_URL \$TMP >/dev/null 2>&1 && \\
  rsync -a --exclude='/.git' --exclude='/README.md' --exclude='/install_KuCat.sh' \$TMP/luci-theme-kucat/htdocs/luci-static/kucat/ $STATIC/ && \\
  rsync -a --exclude='/.git' --exclude='/README.md' --exclude='/install_KuCat.sh' \$TMP/luci-theme-kucat/luasrc/view/themes/kucat/ $LUCI/ && \\
  rm -rf \$TMP
EOF
